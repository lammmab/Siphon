...
static int parse_splits_sections(const char* path, char*** out_names, int* out_count) {
    FILE* f = fopen(path, "rb");
    if (!f) { siphon_log("cannot open %s", path); return -1; }
    char** names = NULL;
    int count = 0, cap = 0;
    char line[1024];
    int in_sections = 0;
...
static int rel_load(RelFile* r, const uint8_t* bytes, size_t n, const char* splits_path) {
    memset(r, 0, sizeof(*r));
    if (n < 0x40) { siphon_log("REL header too small"); return -1; }
    r->data = (uint8_t*)malloc(n);
    memcpy(r->data, bytes, n);
    r->size = n;
    uint32_t num_sections     = gc_be32(r->data + 0x0C);
    uint32_t section_info_off = gc_be32(r->data + 0x10);
    if (section_info_off + num_sections * 8 > n) {
        siphon_log("REL section info out of range");
        return -1;
    }
    r->section_count = (int)num_sections;
    r->sections = (RelSection*)calloc(num_sections, sizeof(RelSection));
    for (uint32_t i = 0; i < num_sections; i++) {
        uint32_t off = gc_be32(r->data + section_info_off + i * 8);
        uint32_t len = gc_be32(r->data + section_info_off + i * 8 + 4);
...
static int resolve_dol(void* ctx, const char* name, uint32_t* file_off, uint32_t* size) {
    ...
    if (!s) { siphon_log("symbol %s not found", name); return -1; }
    if (s->size == 0) { siphon_log("symbol %s has zero size", name); return -1; }
...
static int process_extracts(const GCYamlNode* extract_node, const char* out_dir,
                            const uint8_t* base_data, void* ctx, resolve_fn fn,
                            GCJsonWriter* jw) {
...
static int read_rel(ArcCache* cache, const char* base,
    ...
    if (file_exists(arc_full)) {
        ...
        if (!arc) { siphon_log("cannot open archive %s", arc_full); return -1; }
        ...
        if (found < 0) { siphon_log("%s not found inside %s", inner_path, arc_full); return -1; }
        if (gc_arc_read_file(arc, found, (void**)out_data, out_size) != 0) {
            siphon_log("cannot read entry from %s", arc_full); return -1;
        }
        snprintf(dep_path, dep_n, "%s", arc_full);
        return 0;
    }
    siphon_log("cannot find %s (tried %s and %s)", inner_path, dir_form, arc_full);
    return -1;
}
...
static int maybe_decompress_yaz0(uint8_t** pp, size_t* pn) {
    ...
    if (gc_yaz0_decompress(*pp, *pn, &out, &out_n) != 0) {
        siphon_log("yaz0 decompress failed"); return -1;
    }
    free(*pp);
    *pp = out;
    *pn = out_n;
...
int gc_dol_split(const char* yaml_path, const char* out_dir) {
    ...
    gc_json_begin_array(&jw);
    {
        ...
        if (process_extracts(ex, out_dir, dol.data, &ctx, resolve_dol, &jw) != 0) {
            gc_json_end_array(&jw); gc_json_end_object(&jw);
            fclose(jf);
            gc_symbols_free(&dol_syms); dol_free(&dol); gc_yaml_free(cfg);
        ...
        for (const GCYamlNode* m = mods->children; m; m = m->next) {
            ...
            if (mobj) {
                ...
                if (colon) {
                    ...
                    if (rel_bytes) { snprintf(rel_dep, sizeof(rel_dep), "%s", direct); rel_ok = 1; }
                }
                if (rel_ok && maybe_decompress_yaz0(&rel_bytes, &rel_n) != 0) rel_ok = 0;
                if (rel_ok && rel_n >= 4) module_id_val = gc_be32(rel_bytes);
    ...
    if (df) {
        ...
        for (int i = 0; i < deps.count; i++) fprintf(df, " \\\n  %s", deps.items[i]);
        fputc('\n', df);
        fclose(df);
    }
